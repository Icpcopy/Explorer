<template>
  <div class="container-children-box">
    ...
    >
      ...
      <el-radio-button label="Nft" value="Nft"></el-radio-button>
    ...
    <NftTab v-if="Accountbalance_radio == 'Nft'" />
  </div>
</template>
...
export default {
  components: { Asset, Lssue, Deal, NftTab },
  data() {
    return {
      Accountbalance_radio: "Asset",
      result: { balances: [[]], tokenBalances: [], transferBalances: [] },
    };
  },
  watch: {
    $route(from, to) {
  ...
  methods: {
    getAddressInfoFn() {
      console.log("进来没", this.$route.params.name);
