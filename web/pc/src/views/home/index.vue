      ...
      <div class="section-data">
        <div class="section-tops">
          ...
          <div class="srech-text">
            ...
            <div v-if="WebSocket_data[0].coinInfo">
              ...
              {{
                (WebSocket_data[0] &&
                  WebSocket_data[0].coinInfo &&
                  WebSocket_data[0].coinInfo.coinName &&
                  WebSocket_data[0].coinInfo.coinName.toUpperCase()) ||
                "--"
              }}
            </div>
            <div
        ...
        <div class="section-middel">
          <div class="middel-left">
            ...
            >
              {{ (WebSocket_data[0].height * 1 || 0).toFixed(0) }}
            </h3>
            <div
              class="shubiao"
              @click="$userInfoRouterFn(WebSocket_data[0].operatorAddress)"
            >
              <img class="img_icon" src="../../assets/icplazaLogo.png" />
          ...
          <div class="middel-right">
            <div class="ulrs">
              <div class="margins">
                ...
                <div class="spans">
                  {{ WebSocket_data[0].time_stamp_time }} UTC
                </div>
              </div>
              <div class="margins">
            ...
            <div class="ulrs">
              ...
              <div>
                ...
                <div class="spans" v-if="WebSocket_data[0].coinInfo">
                  {{
                    (
                      (WebSocket_data[0].coinInfo.Bonded /
        ...
        <div class="section-footer">
          <div class="footer-left">
            ...
            <div class="f-l-content">
              <ul v-if="WebSocket_data.length > 1">
                <li v-for="(item, key) of WebSocket_data" :key="key">
                  ...
                  <div
                    class="c-center maodian"
                    @click="$userInfoRouterFn(item.operatorAddress)"
                  >
                    <div>
                      <img
                        class="user_icon home_img"
                        :src="item.validator_coin"
                      />
                    </div>
            ...
            <div class="f-l-footer">
              <div
                class="buttons btn-left"
                @click="routerHomeFn('/blockchain')"
              >
                View all
              </div>
            </div>
          ...
          <div class="footer-right">
                  ...
                  <div v-else class="r-c-center">
                    ...
                    <div class="from-div" v-else>
                      ...
                      >
                        <span class="froms">From</span>
                        <img
                          class="list-img maodian"
                          :src="item.event[0].from.icon"
                        />
            ...
            <div class="r-l-footer">
              <div class="buttons btn-left" @click="routerHomeFn('/deal')">
                View all
              </div>
            </div>
          </div>
...
export default {
  ...
  computed: {
    ...mapState({
      socketAddress: (state) => state.common.socketAddress,
    }),
  },
  watch: {
    socketAddress: {
  ...
  created() {
    // this.WebSocketFn();
    // if (!this.socketAddress) {
    //   this.getUrl({ configKey: "blockchain_browser_websocket_address" });
    // } else {
    this.WebSocketFn();
    // }
  },
  ...
  methods: {
    ...
    WebSocketFn() {
      ...
      // var ws = new WebSocket("ws://192.168.0.86:12345/ws");

      // 生产环境
      // var ws = new WebSocket("wss://browse.ic-plaza.org/ws");
      var ws = new WebSocket("wss://browsemainnet.ic-plaza.org/ws"); //主网
      // var ws = new WebSocket("ws://192.168.2.13:7002/ws"); //new WebSocket("ws://10.107.180.172:12345/ws");
      ...
      ws.onmessage = function (data) {
        ...
        if (WebSocket_data.length) {
          if (WebSocket_data.length == 10) {
            ...
            console.log(This.WebSocket_data[0], "WebSocket_data======");
          }
        }
        if (ws.readyState) {
          // This.loading.close();
