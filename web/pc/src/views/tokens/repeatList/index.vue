<template>
  <div class="icp-repeatTokens">
    <div class="icp-repeatTokens__tip">
      <div class="icp-repeatTokens__tip--text">
        Token Tracker <span>(Total ICT-721 Token:{{ total || 0 }})</span>
      </div>
      <div class="icp-repeatTokens__tip--search">
        <el-input
          style="width: 300px"
          v-model="param"
    ...
    <div class="icp-repeatTokens__table">
      <el-table
        v-loading="loading"
        ref="multipleTable"
        :data="tableData"
        tooltip-effect="dark"
        stripe
        header-cell-class-name="icp-repeatTokens__table--header"
      >
        <el-table-column
          align="center"
        ...
        >
          <template #default="{ row }">
            ...
            <span
              v-if="item.prop == 'transferTime'"
              class="icp-repeatTokens__intro"
            >
              {{
                `${
                  row[item.prop] &&
            ...
            <span v-if="item.prop == 'tokenInfo'">
              <router-link
                :to="{
                  path: `/tokens/detail/${row['contractAddress']}`,
                  query: {
                    type: 'erc721',
                  },
                }"
              >
                ...
                <div class="icp-repeatTokens__table--tokenInfo">
                  <div class="ittt-img">
                  ...
                  <div class="ittt-box">
                    <div class="ittt-title">
                      {{ row["tokenName"] || "--"
                      }}<span v-if="row['tokenSymbol']">({{ row["tokenSymbol"] }})</span>
                    </div>
        ...
        .then((req) => {
          ...
          if (req.code >= 200 && req.code < 300) {
           ...
           this.tableData =result.records
            Tokens.getTokenList({ listType: 2 }).then((res) => {
