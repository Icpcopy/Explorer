  ...
  <div class="container-children-box">
    ...
    <div class="header-state">
      ...
      <div class="hongse">
        ...
        <span
          v-else-if="ProposalByPropos.status == 'PROPOSAL_STATUS_PASSED'"
          class="tx_grren"
        >
          <img class="imgerw" src="../../assets//tpsuccess.png" /> Pass
        </span>
        <span
          v-else-if="ProposalByPropos.status == 'PROPOSAL_STATUS_REJECTED'"
          class="hongse"
        >
        ...
        <span
          v-else-if="
            ProposalByPropos.status == 'PROPOSAL_STATUS_DEPOSIT_PERIOD'
          "
          class="tx_yellow"
          >Deposit period</span
        >
    ...
    <div>
      <header class="s-title">
        Tally Result ————
        <span v-if="ProposalByPropos.status == 'PROPOSAL_STATUS_VOTING_PERIOD'"
          >Voting period</span
        >
        <span v-else-if="ProposalByPropos.status == 'PROPOSAL_STATUS_PASSED'"
          >Pass</span
        >
        <span v-else-if="ProposalByPropos.status == 'PROPOSAL_STATUS_REJECTED'"
          >Reject</span
        >
        <span
          v-else-if="
      ...
      <section class="Progress select-datas-box">
        ...
        <div class="total-box">
          <div class="Total-left">
            <span>Total:</span>
            <span>{{
              (ProposalByPropos.totalVote &&
                onFeixedNum(ProposalByPropos.totalVote, 6)) ||
              0
            }}</span>
            <span>{{ ProposalByPropos.coinName }}</span>
          </div>
          <div
            v-if="ProposalByPropos.status == 'PROPOSAL_STATUS_VOTING_PERIOD'"
            class="Total-right"
          >
            <div class="total-top">
              <div class="total-top-left">
                ...
                <span style="margin-left: 5px"
                  >{{
                    (ProposalByPropos.CurrentTurnout &&
                      onFeixedNum(ProposalByPropos.CurrentTurnout * 100, 2)) ||
                    0
                  }}%</span
                >
              </div>
            ...
            <div class="total-bottom">
              ...
              of
              {{
                (ProposalByPropos.bondedTokens &&
                  onFeixedNum(ProposalByPropos.bondedTokens, 6)) ||
                0
              }}
              has voted
            </div>
          ...
          <li>
            ...
            <div>
              <span class="top-title">
                {{
                  final_tally_result.yes
                    ? onFeixedNum(
                        final_tally_result.yes / 1000000000000000000,
                        2
                      )
                    : final_tally_result.yes
                }}（Threshold {{ ProposalByPropos.yesRate }}%）
              </span>
              <el-progress
                :text-inside="true"
                :stroke-width="15"
              ...
              <span class="top-title">
                {{
                  final_tally_result.abstain
                    ? onFeixedNum(
                        final_tally_result.abstain / 1000000000000000000,
                        2
                      )
                    : final_tally_result.abstain
                }}
                （Threshold{{ ProposalByPropos.abstainRate }}%）
              </span>
              <el-progress
                :text-inside="true"
                :stroke-width="15"
              ...
              <span class="top-title">
                {{
                  final_tally_result.no
                    ? onFeixedNum(
                        final_tally_result.no / 1000000000000000000,
                        2
                      )
                    : final_tally_result.no
                }}
              ...
              <span class="top-title">
                {{
                  final_tally_result.no_with_veto
                    ? onFeixedNum(
                        final_tally_result.no_with_veto / 1000000000000000000,
                        2
                      )
                    : final_tally_result.no_with_veto
                }}
  ...
  methods: {
    getProposalByProposalIdFn() {
      getProposalByProposalId({
        ...
        console.log(this.ProposalByPropos, "this.ProposalByPropos=====");
        ...
        this.ProposalByPropos.noRate = (
          this.ProposalByPropos.noRate * 100
        ).toFixed(2);
        this.ProposalByPropos.yesRate = (
          this.ProposalByPropos.yesRate * 100
        ).toFixed(2);
