...
export default {
  ...
  created() {
    var pages = JSON.parse(sessionStorage.getItem("ProposedtransactionIndex"));
    if (pages != null) {
  ...
  methods: {
    ...
    currpageChange(val) {
      this.pageIndex = val;
      sessionStorage.setItem("ProposedtransactionIndex", val);
    ...
    getValidatorRelatedTxFn() {
      var pages = JSON.parse(
        sessionStorage.getItem("ProposedtransactionIndex")
      );
      if (pages != null) {
        this.pageIndex = pages;
      }
      getValidatorRelatedTx({
        ...
        this.anmot = res.result.coinName.toUpperCase();
