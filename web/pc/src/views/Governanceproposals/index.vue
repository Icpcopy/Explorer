      ...
      >
        ...
        <el-table-column prop="address" label="Status" align="center">
          <template #default="scope">
            ...
            >
            <span
              v-else-if="scope.row.status == 'PROPOSAL_STATUS_PASSED'"
              class="tx_grren"
              >Passed</span
            >
            <span
              v-else-if="scope.row.status == 'PROPOSAL_STATUS_REJECTED'"
              class="hongse"
              >Rejected</span
            >
            <span
              v-else-if="scope.row.status == 'PROPOSAL_STATUS_DEPOSIT_PERIOD'"
              class="tx_yellow"
              >Deposit period</span
            >
        ...
        <el-table-column prop="address" label="Total Deposit" align="center">
          <template #default="scope">
            <span v-if="scope.row.total_deposit[0]"
              >{{
                (scope.row.total_deposit[0].amount &&
                  scope.row.total_deposit[0].amount / 1000000000000000000 &&
                  onFeixedNum(
                    (scope.row.total_deposit[0].amount / 10000000000000000000) *
                      10,
                    6
                  )) ||
                0
...
export default {
  ...
  data() {
    return {
      tableData: [{ total_deposit: [{ amount: 0 }] }],
      pageIndex: 1,
      pageSize: 20,
      total: 0,
      coinName: "",
