  ...
  <div class="container">
    ...
    <el-radio-group v-model="bigType" size="medium" @change="bigTypeFn">
      <el-radio-button label="transfer"
        >Transfer ({{ AllTypeCountData.transferCount }})</el-radio-button
      >
      <el-radio-button label="Staking"
        >Staking ({{ AllTypeCountData.delegatorCount }})</el-radio-button
      >
      <el-radio-button label="reward"
        >Distribution({{ AllTypeCountData.rewardCount }})</el-radio-button
      >
      <el-radio-button label="vote"
        >Governance({{ AllTypeCountData.voteCount }})</el-radio-button
      >
      <el-radio-button label="jail"
        >Slashing({{ AllTypeCountData.jailCount }})</el-radio-button
      >
      <el-radio-button label="nft"
        >Nft({{ AllTypeCountData.nftCount || 0 }})</el-radio-button
      >
      <!-- <el-radio-button label="ibc"
        >IBC({{ AllTypeCountData.ibcCount }})</el-radio-button
      > -->
      <!-- <el-radio-button label="ethbridge"
        >EthBridge({{ AllTypeCountData.ethBridgeCount }})</el-radio-button
      > -->

      <el-radio-button label="issues"
        >Issue({{ AllTypeCountData.issuesCount }})</el-radio-button
      >
      <el-radio-button label="defi"
        >Defi({{ AllTypeCountData.defiCount }})</el-radio-button
      >
      <!-- <el-radio-button label="ammswap"
        >Swap({{ AllTypeCountData.ammswapCount }})</el-radio-button
      > -->

      <!-- <el-radio-button label="business"
        >Order({{ AllTypeCountData.businessCount }})</el-radio-button
      > -->
    </el-radio-group>
    <div class="jiazai-box" v-if="jiazaibool">
      <img src="../../../assets/jiazai.gif" />
    </div>
    <div class="Transction-table" v-if="jiazaibool == false">
      <el-table
        :data="Table_data"
        style="width: 100%"
      ...
      >
            ...
            <span v-else-if="scope.row.event[0].amount">
              {{
                (scope.row.event[0].amount &&
                  onFeixedNum(scope.row.event[0].amount, 6)) ||
                0
              }}
        ...
        <el-table-column prop="address" label="Fee" align="center">
          <template #default="scope">
            <span
              >{{ (scope.row.fee && onFeixedNum(scope.row.fee, 6)) || 0 }}
              {{ scope.row.event[0].coinName }}</span
            >
          </template>
        </el-table-column>
...
export default {
  ...
  created() {
    ...
    var bigtype = sessionStorage.getItem("bigtype");
    if (bigtype != null && bigtype != "null") {
  ...
  methods: {
    ...
    bigTypeFn(value) {
      this.bigType = value;
      sessionStorage.setItem("bigtype", value);
      this.pageIndex = 1;
      this.getByBlockNumberFn();
    },
    getByBlockNumberFn(pageIndex, type) {
      this.jiazaibool = true;
      getByBlockNumber({
