          ...
          <template #default="scope">
            ...
            <div v-else>
              <div v-if="scope.row.event[0].fromIsNull == 2">
                <span>{{
                  (scope.row.event[0].from.selfBonded &&
                    onFeixedNum(scope.row.event[0].from.selfBonded, 6)) ||
                  0
                }}</span>
              </div>
              <div v-else-if="scope.row.event[0].toIsNull == 2">
                <span>{{
                  (scope.row.event[0].to.selfBonded &&
                    onFeixedNum(scope.row.event[0].to.selfBonded, 6)) ||
                  0
                }}</span>
              </div>
...
export default {
  ...
  created() {
    var pages = JSON.parse(sessionStorage.getItem("VerifierverificationIndex"));
    if (pages != null) {
  ...
  methods: {
    ...
    currpageChange(val) {
      this.pageIndex = val;
      sessionStorage.setItem("VerifierverificationIndex", val);
    ...
    getValidatorRelatedTxFn() {
      var pages = JSON.parse(
        sessionStorage.getItem("VerifierverificationIndex")
      );
      if (pages != null) {
        this.pageIndex = pages;
      }
