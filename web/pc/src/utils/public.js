...
export function  headerRouterFn(addr,refresh) {
      ...
      if (reg.test(addr) || regs.test(addr) || regs_themis.test(addr) || regs_usdg.test(addr) || regs_igpc.test(addr) || regs_fec.test(addr) ) {
            pathName = "/Validatorinfo/"
      } else {
            pathName ="/Accountdetails/" // "/tokens/addressdetail/"
      }
      this.$router.push({
            path:pathName+addr
      })
...
export function jiequStrFnMaxlength (str) {  
      if(str){
          if(str.length > 10){
              var start_str = str.slice(0,10);
           var end_str = str.substr(str.length-10,10);
           return start_str +"......" +end_str;
          }else{
              return str;
          }
