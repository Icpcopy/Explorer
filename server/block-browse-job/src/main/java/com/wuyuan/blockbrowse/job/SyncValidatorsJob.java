...
//    private String chainPrefix;
    ...
    public void getValidators(){
        ...
        BigDecimal bondedToken = new BigDecimal(bondedTokens).divide(new BigDecimal(Math.pow(10,configService.getChaindecimal())));//获取总的委托金额
...
//                String totalDelegations = JSON.parseObject(res).getJSONObject("pagination").getString("total");
                ...
                tokenNumber = new BigDecimal(tokenNumber).divide(new BigDecimal(Math.pow(10,ConfigUtil.ChainDecimal))).toPlainString();
                String votingPower = new BigDecimal(tokenNumber).divide(bondedToken,ConfigUtil.ChainDecimal, RoundingMode.HALF_UP).toPlainString();
                ...
                if (selfDelegateAmount == null || selfDelegateAmount.containsKey("code")){
                    ...
                    String delegateAmount = new BigDecimal(selfDelegateAmount.getJSONObject("delegation_response").getJSONObject("balance").getString("amount")).divide(new BigDecimal(Math.pow(10,configService.getChaindecimal())),configService.getChaindecimal(),RoundingMode.HALF_UP).toPlainString();//单位转换
                    if (validatorDelegatorService.isExist(selfDelegateAddress,operatorAddress) == 0){
                        validatorDelegatorService.saveDelegator(selfDelegateAmount.getJSONObject("delegation_response"));
                    }
                ...
                BigDecimal commissionRewards = BalanceUtil.getValidateatorReward(operatorAddress,configService.getConfig(ConfigUtil.chainUrlKey)+"/"+configService.getChainPrefix(),configService.getChainName(),configService.getChaindecimal());
