...
public class AmountUtil {
    ...
    public static AmountUtil initAmount(String amountStr) {
        if (StringUtils.isNotBlank(amountStr)) {
            AmountUtil amountUtil = new AmountUtil();
            String coin = getCoin(amountStr);
            amountUtil.setCoin(coin);
            amountUtil.setAmount(getAmount(amountStr, coin));
            return amountUtil;
        }
        AmountUtil amountUtil = new AmountUtil();
        amountUtil.setCoin("");
    ...
    public static BigDecimal getAmount(String amountStr,String coin){
        ...
        if(StringUtils.isNotBlank(amountStr)){
            ...
            amount=new BigDecimal(amountStr.substring(0,index));
            return amount;
        }
    ...
    public static BigDecimal getAmount(String amountStr,String coin,BigDecimal decimal,int scale){
        ...
        return new BigDecimal(amountStr).divide(decimal,scale,BigDecimal.ROUND_HALF_UP);
    }
    ...
    public static BigDecimal getAmoutByDecimal(BigDecimal amount,int decimal){
        return amount.divide(new BigDecimal(Math.pow(10,decimal)), decimal, BigDecimal.ROUND_HALF_UP);
    }
