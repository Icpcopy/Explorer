...
public class BalanceUtil {
    ...
    public static BigDecimal getDelegatorBalance(String address, String url,int decimal)  {
        BigDecimal balance = BigDecimal.ZERO;
        try {
            List<String> list = getValidators(address, url).toJavaList(String.class);
            balance = getPledgedBalance(address, list, url).divide(new BigDecimal(Math.pow(10,decimal)), decimal, BigDecimal.ROUND_HALF_UP);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return balance;
    ...
    public static BigDecimal getRewardBalance(String address, String url,int decimal) {
        try {
            ...
            if(rewards!=null&&rewards.size()>0){
                for (int i=0;i<rewards.size();i++) {
                    ...
                    return amount.getBigDecimal("amount").divide(new BigDecimal(Math.pow(10,decimal)),decimal,BigDecimal.ROUND_HALF_UP);
                }
            }
    ...
    public static BigDecimal getValidatorRewardBalance(String address, String url,String coinName,int decimal) {
        try {
            String uri = url + "/distribution/v1beta1/validators/"+ address +"/commission";
            String res = HttpUtils.httpGet(uri);
            JSONObject json = JSONObject.parseObject(res);
            JSONArray commission = json.getJSONObject("commission").getJSONArray("commission");
            BigDecimal reward = BigDecimal.ZERO;
            if(commission != null && commission.size() > 0){
                for (int i=0;i<commission.size();i++) {
                    JSONObject amount = commission.getJSONObject(i);
                    if (amount.getString("denom").equals(coinName)){
                        reward = reward.add(amount.getBigDecimal("amount"));
                    }
                }
            }
            JSONArray team_commission = json.getJSONObject("commission").getJSONArray("team_commission");
            if(team_commission != null && team_commission.size() > 0){
                for (int i=0;i<team_commission.size();i++) {
                    JSONObject amount = team_commission.getJSONObject(i);
                    if (amount.getString("denom").equals(coinName)){
                        reward = reward.add(amount.getBigDecimal("amount"));
                    }
                }
            }
            return reward.divide(new BigDecimal(Math.pow(10,decimal)),decimal,BigDecimal.ROUND_HALF_UP);
        } catch (Exception e) {
     ...
     * @return
     */
    public static BigDecimal getValidateatorReward(String address,String url, String coin,int decimal) {
        String uri = url + "/distribution/v1beta1/validators/"+address+"/commission";
        String res = HttpUtils.httpGet(uri);
        JSONObject json=JSONObject.parseObject(res);
        if (json != null){
