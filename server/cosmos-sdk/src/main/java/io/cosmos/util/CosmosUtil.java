...
public class CosmosUtil {
    ...
    public static JSONObject getUsedBalances(String url, String addresss) {
        String uri = url + "/bank/v1beta1/spendable_balances/" + addresss;
        String res = HttpUtils.httpGet(uri);
        if (res == null) {
            return null;
        }
...
//    }
    ...
    public static String getCommunity(String url) {
        ...
        if (StringUtils.isNotBlank(res)) {
            JSONArray pool = JSON.parseObject(res).getJSONArray("pool");
            if (pool.isEmpty()) {
                return "0";
            }
            String community = String.valueOf(pool.getJSONObject(0).get("amount"));
            community = new BigDecimal(community).divide(new BigDecimal(Math.pow(10, decimal)),6,RoundingMode.HALF_UP).toPlainString();
            return community;
        }
        return "0";
    }
    ...
    public static String getTotal(String url, String coinName) {
        ...
        if (StringUtils.isNotBlank(result)) {
            ...
            return new BigDecimal(total).divide(new BigDecimal(Math.pow(10, decimal)),6, RoundingMode.HALF_UP).toPlainString();
        } else {
            return "0";
        }
    }
    public static String getBurn(String url, String coinName) {
        String uri = url + "/bank/v1beta1/deflation" ;
        String result = HttpUtils.httpGet(uri);
        if (StringUtils.isNotBlank(result)) {
            JSONArray  deflations = JSON.parseObject(result).getJSONArray("deflation");//获取Ato的总发行数
            for (int i=0;i<deflations.size();i++){
                JSONObject deflation = deflations.getJSONObject(i);
                if(coinName.equalsIgnoreCase(deflation.getString("denom"))){
                    return deflation.getString("amount");
                }
            }
        }
        return "0";
    ...
    public static void main(String[] args) {
        JSONObject usedBalances = getUsedBalances("http://192.168.3.23:1317/icplaza", "icplaza1cvam2hajww2c8aa6m35q378c4f637wc2nknndn");
        System.out.println(usedBalances);
    }

    public static JSONObject getNftList(String url, Integer pageSize, Integer pageIndex) {
        Integer offsetNum = (pageIndex - 1) * pageSize;

        String uri = url + "/unique/nft/collections?pagination.offset=" + offsetNum + "&pagination.limit=" + pageSize + "&pagination.count_total=true";
        return JSON.parseObject(HttpUtils.httpGet(uri), Feature.DisableSpecialKeyDetect);
    }

    public static List<JSONObject> getBlockResultes(String url, long height) {
        String uri = url + "/block_results?height=" + height;
        String data = HttpUtils.httpGet(uri);
        JSONObject result = JSON.parseObject(data);
        if(result==null){
            return new ArrayList<>();
        }
        JSONArray endBlockEvents = result.getJSONObject("result").getJSONArray("begin_block_events");
//        System.out.println(endBlockEvents);
        if (endBlockEvents != null) {
            List<JSONObject> endList = endBlockEvents.toJavaList(JSONObject.class);
            endList.stream().forEach(events -> {
                String type = events.getString("type");
                JSONArray attributes = events.getJSONArray("attributes");
                attributes.toJavaList(JSONObject.class).stream().forEach(attribute -> {
                    String key = attribute.getString("key");
                    attribute.put("key", CosmosUtil.base64Encode(key));
                    String value = attribute.getString("value");
                    attribute.put("value", CosmosUtil.base64Encode(value));
                });
            });
            return endList;
        }
    ...
    public static final Base64.Decoder decode = Base64.getDecoder();
    public static String base64Encode(String base64Str) {
        if (StringUtils.isNotBlank(base64Str)) {
            return new String(decode.decode(base64Str));
        }
        return base64Str;
    }
}
