...
public class ETHTransferTxService {
    ...
    public long isExistTx(String fromHash,String eventNonce){
        Criteria criteria = new Criteria();
        Criteria from = Criteria.where("fromHash").is(fromHash);
        Criteria to = Criteria.where("eventNonce").is(eventNonce);
...
//    }
    ...
    public void dealEthTOGauss(JSONObject tx, EthBlock.Block ethBlock){
        ...
        ethSendGauss.put("toChain","usdg");
        ethSendGauss.put("memo","");
        ...
        String date = DateFormatUtil.timeToUTCFormat(timeStamp.longValue() * 1000);//毫秒转日期
                    ...
                    try {
                        ...
                        if (StringUtils.isNotBlank(toData) && toData.contains(data)){
                            ...
                            if (eventNonce.compareTo(getEventNonce(toData)) == 0){
                                ...
                                if (isExistTx(fromHash, String.valueOf(eventNonce)) == 0){
                                    saveTx(ethSendGauss);
    ...
    public void dealSendToEth(JSONObject tx,Integer code,String hash){
        ...
        sendToEth.put("memo",tx.getString("memo"));


        sendToEth.put("fromHash",hash);
        sendToEth.put("toHash",null);
        sendToEth.put("fromChain","usdg");
