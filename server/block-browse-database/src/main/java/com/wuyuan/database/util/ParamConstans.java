...
public class ParamConstans {
    ...
    public static String getBurn(String url,String prefix,String coinName){
        ParamConstans burnJson=ParamConstans.getParamConstans("burn");
        String burn = null;
        if (burnJson != null){
            if ( (StringUtils.isBlank(burnJson.value) || burnJson.time < System.currentTimeMillis()-5*1000*60) && !totalFlag ){
                new Thread(() -> {
                    totalFlag = true;
                    getTotal(url,prefix,coinName);
                    totalFlag = false;
            ...
            burn=burnJson.getValue();
        }else {
            burn = getBurns(url,prefix,coinName);
        }
        return burn;
    }
    ...
    public static String getPoolChane(String url,String prefix){
        ...
        if(poolJsonParam == null){
            ...
            if ( (StringUtils.isBlank(poolJsonParam.value) ||"0".equals(poolJsonParam.value)|| poolJsonParam.time < System.currentTimeMillis()-5*1000*60) && !poolFlag ){
                new Thread(() -> {
                    poolFlag = true;
                    getPool(url,prefix);
                    poolFlag = false;
    ...
    public static String getBurns(String url,String prefix,String coinName){
        String burn = CosmosUtil.getBurn(url +"/"+prefix,coinName);
        ParamConstans paramConstans=new ParamConstans();
        paramConstans.setTime(System.currentTimeMillis());
        paramConstans.setValue(burn);
        ParamConstans.put(paramConstans,"burn");
        return burn;
    }
