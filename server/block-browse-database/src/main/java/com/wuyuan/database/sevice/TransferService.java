...
public class TransferService {
    ...
    public static JSONObject getTransfer(JSONObject transfer, String chainName, String url,int chaindecimals){
        ...
        if (fee.size() > 0){
            ...
            if ("udga".equalsIgnoreCase(demno)){
                ...
                transaction.put("fee",new BigDecimal(fee.getJSONObject(0).getString("amount"))
                        .divide(new BigDecimal(Math.pow(10,chaindecimals))).toPlainString());
...
//                case Transfer.DepositClaim://eth->gauss
...
//                    break;
                case Transfer.SubmitEthereumEvent://eth->gauss
                    txJson.put("type","SubmitEthereumEvent");
                    JSONObject ethEvent = value.getJSONObject("event");
                    if (ethEvent == null) {
                        break;
                    }
                    String eventType = ethEvent.getString("@type");

                    if ( eventType.equals("/gravity.v1.SendToCosmosEvent")) {
                        txJson.put("type","DepositClaim");
                        String ethSend = ethEvent.getString("ethereum_sender");
                        String receiver = ethEvent.getString("cosmos_receiver");
                        long ethBlockHeight = ethEvent.getLongValue("ethereum_height");
                        long eventNonce = ethEvent.getLongValue("event_nonce");
                        ...
                        String contractAddress = ethEvent.getString("token_contract");
                        txJson.put("contractAddress",contractAddress);
                        JSONObject contract = getContract(contractAddress);
                        int decimal = contract.getIntValue("decimals");
                        amount = ethEvent.getBigDecimal("amount").divide(new BigDecimal(Math.pow(10,decimal))).setScale(decimal).toPlainString();
                        txJson.put("decimal",decimal);
                        txJson.put("amount",amount);
                        txJson.put("coinName",contract.getString("symbol"));
                    }else if (eventType.equals("/gravity.v1.BatchExecutedEvent")){
                        txJson.put("type","WithdrawClaim");

                        long eventNonce = ethEvent.getLongValue("event_nonce");
                        long ethBlockHeight = ethEvent.getLongValue("ethereum_height");
                        String contractAddress = ethEvent.getString("token_contract");
                        long bathNonce = ethEvent.getLongValue("batch_nonce");
                        String orchestrator = value.getString("signer");
                        JSONObject contract = getContract(contractAddress);
                    ...
                    break;
                case Transfer.SendToEth://gauss->eth
                    txJson.put("type","SendToEth");
                    String sender = value.getString("sender");
                    String ethDest = value.getString("ethereum_recipient");
                    ...
                    if (denom.startsWith("gravity")){
                        ...
                        JSONObject contract = getContract(denom);
...
//                    }
//                    break;
                case Transfer.SubmitEthereumTxConfirmation://gauss跨eth打包确认
                    txJson.put("type","SubmitEthereumTxConfirmation");

                    ethEvent = value.getJSONObject("confirmation");
                    if (ethEvent != null && ethEvent.getString("@type").equals("/gravity.v1.BatchTxConfirmation")){
                        txJson.put("type","ConfirmBatch");
                        String contractAddress = ethEvent.getString("token_contract");
                        JSONObject contract = getContract(contractAddress);
                        txJson.put("contractAddress",contractAddress);
                        ...
                        String ethAddress = ethEvent.getString("ethereum_signer");
                        long nonce = ethEvent.getLong("batch_nonce");
                            ...
                            logs.toJavaList(JSONObject.class).stream().forEach(log ->{
                                log.getJSONArray("events").toJavaList(JSONObject.class).stream().forEach(event -> {
                                    if("message".equals(event.getString("type"))){
                                        ...
                                        for (int i = 0; i < attributes.size(); i++) {
                                            JSONObject attribute = attributes.getJSONObject(i);
                                            if ("outgoing_tx_id_list".equals(attribute.getString("key"))){
                                                String outgoingTxIds = attribute.getString("value");
                                                txJson.put("outgoingTxIds",outgoingTxIds);
                                            }
                                        }
