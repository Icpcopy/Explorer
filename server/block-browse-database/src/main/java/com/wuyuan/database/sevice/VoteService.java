...
//    private String chainPrefix;
    ...
    public JSONObject getProposalByProposalId(String proposalId){
        ...
        List<JSONObject> tx = null;
        try {
            tx = transactionService.getTransaction(null,null,"submitproposal",null,proposalId,null,0,1,1,null,null,null).getRecords();
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        BigDecimal total = totalVote.divide(new BigDecimal(Math.pow(10,configService.getChaindecimal())));
        proposal.put("totalVote",total.toPlainString());
        proposal.put("mTotalVote",total.divide(new BigDecimal(Math.pow(10,configService.getChaindecimal())),0,RoundingMode.UP).toPlainString());
        if (proposal.getString("status").equals("PROPOSAL_STATUS_VOTING_PERIOD")){
            ...
            BigDecimal boned = new BigDecimal(bondedTokens).divide(new BigDecimal(Math.pow(10,configService.getChaindecimal())));
            proposal.put("bondedTokens",boned.toPlainString());
            proposal.put("mBondedTokens",boned.divide(new BigDecimal(Math.pow(10,configService.getChaindecimal())),0,RoundingMode.UP).toPlainString());
        ...
        if (totalVote .compareTo(BigDecimal.ZERO) != 0){
        ...
        proposal.put("coinName",configService.getCoinName1());
        proposal.put("chainName",configService.getChainName());
        proposal.put("yesRate",yesRate.toPlainString());
        proposal.put("noRate",noRate.toPlainString());
        proposal.put("noWithVoteRate",noWithVoteRate.toPlainString());
        proposal.put("abstainRate",abstainRate.toPlainString());
        return proposal;
    }
