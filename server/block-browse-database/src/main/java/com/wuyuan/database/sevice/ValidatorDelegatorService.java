...
//    private String chainName;
    ...
    public void deleteDelegatorByAddress(String address){
        Criteria delegator = Criteria.where("delegation.delegator_address").is(address);
        Query query = new Query(delegator);
        mongoTemplate.remove(query,getCollectionName());
    }
    ...
    public PageModel<JSONObject> getDelegatorByOperatorAddress(String operatorAddress, Integer pageIndex, Integer pageSize){
        ...
        pageModel.setCoinName(configService.getCoinName1());
        return pageModel;
    }

    public List<JSONObject> getUnDelegatorsAll(){
        return mongoTemplate.find(new Query(),JSONObject.class,getUnCollectionName());
    }
...
//    }
    ...
    public long isExistByAddress(String address) {
        Query query = new Query(Criteria.where("delegation.delegator_address").is(address));
        return mongoTemplate.count(query.limit(1),getCollectionName());
    }
