...
public class TransactionApiController {
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "bigType", value = "大交易类型(transfer(交易),staking(委托),reward(收益),vote(治理),jail(出入狱)),business(撮合),issues(合约资产),ibc(跨链),ethbridge（eth跨链），defi（defi）,ammswap(ammswap)", required = false, dataType = "String"),
    ...
    public ApiResult getTransactionByBlockNumber(String blockHeight,String bigType,String smallType,String address,String proposalId,String voteType,Integer status,Integer pageIndex, Integer pageSize,String startTime,String endTime,Integer isAccording) throws Exception {
    ...
    public ApiResult getAllTypeCount(String address,String blockHeight) throws Exception {
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "startTime", value = "开始时间(格式必须为xxxx-xx-xx)", required = false, dataType = "String"),
            @ApiImplicitParam(name = "endTime", value = "结束时间(格式必须为xxxx-xx-xx)", required = false, dataType = "String"),
            @ApiImplicitParam(name = "proposalId", value = "提案id(大类型为deposit时传)", required = false, dataType = "String"),
    })
    @GetMapping("getValidatorRelatedTx")
    public ApiResult getValidatorRelatedTx(String bigType,String smallType,String startTime,String endTime,Integer status,Integer pageIndex,Integer pageSize,String proposalId) {
        ...
        return new ApiResult(200,"查询成功",transactionService.getValidatorRelatedTx(bigType.trim(),smallType,startTime,endTime,status,pageIndex,pageSize,proposalId));
    }
