...
public class BlockApiController {
    ...
    @ApiOperation(value = "获取链上的txNum,addressNum,blockMax", nickname = "查询区块")
    @GetMapping("getChainInfo")
    public ApiResult getChainInfo() {
        JSONObject data = new JSONObject();
        String blockMax = blockService.getMaxBlockNumber();
        Long txNum = transactionService.getTXCount();
        Long addressNum = addressService.getAddressCount();
        data.put("blockMax",blockMax);
        data.put("txNum",txNum);
        data.put("addressNum",addressNum);
