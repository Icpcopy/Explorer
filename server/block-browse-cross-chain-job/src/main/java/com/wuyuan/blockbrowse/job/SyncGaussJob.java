    ...
    public void saveTransaction(JSONArray txs){
        ...
        transactions.stream().forEach(txJson ->{
            ...
            if (null != transfer){
                ...
                JSONObject tx = TransferService.getTransfer(transfer,chainName,configService.getConfig(ConfigUtil.gaussUrl),configService.getChaindecimal());
                ...
                events.toJavaList(JSONObject.class).stream().forEach(event -> {
                    ...
                    switch (type){
                        ...
                        case "ConfirmBatch":
                            code = tx.getInteger("code");
                            long batchNonce = tx.getLongValue("batchNonce");
                            if (code != null && code == 0){
                        ...
                        case "WithdrawClaim":
                            code = tx.getInteger("code");
                            if (code != null && code == 0){
                                batchNonce = event.getLongValue("batchNonce");
                                if (ethBatchService.isExistByNonce(batchNonce) > 0){
